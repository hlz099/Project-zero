...
// Find ingredients "AL(cr)" and "HTPB+Curative"
...
for i=0:Ingredients_getSize(ingredients)-1
    ...
    if Component_getName(ing)=="AL(cr)" then
        c_al = ing;
    end
    if Component_getName(ing)=="HTPB+Curative" then
        c_htpb = ing;
    end
end

if ~Pointer_isComponent(c_al) then
    disp("Could not find AL(cr)!");
    exit;
end
if ~Pointer_isComponent(c_htpb) then
    disp("Could not find HTPB+Curative!");
    exit;
end
...
xlabel("AL(cr) rate","font_style",8,"fontsize",2)
